...
export const AwgInboundSettingsSchema = z
  .object({
    ...
    obfLevel: z.number().int().min(1).max(4).default(2),
    mimicryProfile: z.enum(['tls', 'quic', 'sip', 'dns']).default('tls'),
    ...
    h1: z
      .string()
      .regex(/^$|^\d+(-\d+)?$/, { message: 'pages.inbounds.form.awgHInvalid' })
      .default(''),
    h2: z
      .string()
      .regex(/^$|^\d+(-\d+)?$/, { message: 'pages.inbounds.form.awgHInvalid' })
