#!/bin/bash
# ╔══════════════════════════════════════════════════════════════╗
# ║  ⚠ ЭТОТ СКРИПТ УСТАРЕЛ — ИСПОЛЬЗУЙ НОВУЮ ДВУХЭТАПНУЮ УСТАНОВКУ ⚠
# ║                                                              ║
# ║  Этап 1: bash install-3xui-mirror.sh                         ║
# ║  Этап 2: bash install-lucx-patch.sh                          ║
# ║                                                              ║
# ║  Подробнее: README.md или LucX-UI_DOCS.md                    ║
# ╚══════════════════════════════════════════════════════════════╝
# 
# Продолжение через 3 секунды...
sleep 3
...
install_base() {
    case "${release}" in
        ubuntu | debian | armbian)
            apt-get update && apt-get install -y -q cron curl tar tzdata socat ca-certificates openssl nftables nftables
            ;;
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ "$(printf '%s\n' "$min_version" "$tag_version_numeric" | sort -V | head -n1)" != "$min_version" && "${tag_version_numeric}" != *"-"* ]]; then
            echo -e "${red}Please use a newer version (at least v0.1.0). Exiting installation.${plain}"
            exit 1
        fi
