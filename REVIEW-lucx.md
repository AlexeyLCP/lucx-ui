...
| | Находка | Статус |
|---|---|---|
| P0-1 | Go не верифицирован в песочнице | частично снято: новый код прогнан на Go 1.24 (114 тестов, `-race`), весь модуль — за `make verify` / CI |
...
`go.dev`, `dl.google.com`, зеркала). Поэтому **`go build`, `go vet`,
`golangci-lint`, `go test -race` по всему `internal/` не выполнялись** — вся
Go-часть ниже разобрана только чтением кода.

> **Частично снято при исправлении.** В apt-репозитории нашёлся
> `golang-1.24-go`; распакованный локально (без установки), он позволил
> собрать `internal/lucx/tunnel` целиком против заглушек `config`/`model`/
> `logger` и прогнать `go build` + `go vet` + `go test -race -shuffle=on` —
> 104 теста, включая все существовавшие тесты пакета. Отдельно так же прогнаны
> новые функции из `internal/web/service`. Модульная сборка целиком (с
> внешними зависимостями) по-прежнему требует `make verify` / CI: модульный
> прокси тоже недоступен.
