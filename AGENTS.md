...
- **Fix (lucx.54):** Поле добавлено в **5 точках** в `model.go` для full round-trip: `Client` struct (json tag) + `ClientRecord` struct (gorm column `awg_advanced_security`) + `ToRecord()` (copy) + `ToClient()` (copy) + merge logic (`incoming.AdvancedSecurity && !existing.AdvancedSecurity`, advisory — «true wins, never silently clear»). AutoMigrate добавит колонку на следующем старте.
