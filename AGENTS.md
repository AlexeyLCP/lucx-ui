...
### Pattern 1c: AWG outbounds «не подключаются» после апгрейда панели (рассинхон модуля)
- **Cause:** Мажорный апгрейд upstream amneziawg-модуля (как AWG1 → AWG3 в lucx.50, `v3.0.20260731`) требует **пересборки DKMS-модуля**. `bin/install-awg-module.sh` делает это, но при обновлении через **веб-панель** может возникнуть рассинхон: новый бинарник стартует (с новой миграцией настроек), а модуль ещё не пересобран / awg-quick держит старое состояние → reconcile awgo-N попадает в окно, где параметры расходятся с upstream-сервером, handshake не проходит. Симптом: «подключение есть, но не подключается», откат на прошлый релиз лечит. Обновление через **консольное меню `x-ui`** прогоняет полный install-скрипт единым проходом (модуль + бинарник + БД) → рассинхона нет.
- **Fix:** После любого релиза, меняющего AWG-модуль (проверяй `install-awg-module.sh` в diff релиза), обновлять через **консоль** (`x-ui update` или `bash <(curl .../install.sh)`), а не через веб-панель. Если уже обновил через веб и отвалилось — повторить `bash <(curl -fL https://raw.githubusercontent.com/AlexeyLCP/lucx-ui/main/install.sh)`, это пересоберёт модуль и согласует состояние.
- **Наблюдали:** тестер Александр при апгрейде до lucx.50 через веб-панель — AWG 1.5 outbounds не подключались; повтор через консольное меню `x-ui` всё починил (2026-08-01).
- **TODO (улучшение):** веб-обновление панели должно либо форсировать пересборку модуля, либо предупреждать «этот релиз меняет AWG-модуль — обнови через консоль».
