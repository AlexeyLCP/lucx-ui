...
- **Пресеты обфускации (lucx.136, канон Amnezia):** `GenerateAWGParams` перекалиброван под `AwgInstaller::generateAwgParameters` (amnezia-client): Lite/Standard/Pro вокруг Jc=4..6, Jmin=10, Jmax=50, S1/S2=12..149, S3=12..63, **S4=12 фикс**. Инвариант полный (`isPacketSizeEqual`): 148+S1, 92+S2, 64+S3, 32+S4 попарно различны. H1–H4 по версии: «3»/«3.1» → «1/2/3/4» (HPK шифрует заголовок), «2» → узкие диапазоны шириной ≤100000 (`hBand`), «1.5» → одиночные. Дефолт `mimicryProfile` = `tls` (QUIC-паддинг до 1200 байт давал «000000»-простыни).
