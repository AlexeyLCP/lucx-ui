...
- **Fix (lucx.91), 3 слоя:**
  1. `awgAllowedIPsStale` (client_awg.go): все записи single-host (/32 или /128) И хотя бы одна вне текущей подсети инбаунда → stale. Кастомные (0.0.0.0/0, не-host) не трогаются никогда. Ре-аллокация прямо в `defaultAwgClients` при attach — ключи/PSK сохраняются, ротируется только адрес.
  2. Стартовая миграция `migrateAwgStaleClients` (internal/database, в LUCX-HOOK db.go): чинит УЖЕ сохранённых протухших клиентов без ручного переприсоединения — ре-аллокация из текущей подсети (исключая занятые IP + awgo-аутбаунды), синхронизация `wg_allowed_ips` в clients-таблице (только если там ещё старое значение). Идемпотентна.
