...
- **Fix (lucx.163):** pin `qwdtt-linux-*` to SpaceNeuroX SHA `6c2f7a627d9fc0b54240035818ff86b6d2b6c76f` (`v1.4.2` `./server`). Live binary is `third_party/sidecars/linux-amd64/qwdtt-linux-amd64.gz` (install/update fetch). `release.yml` / `sourcecraft-release.sh` build from that SHA, not Ex3-ui. CLI flags (`-listen/-password/-dns/…`) unchanged. **lucx.218:** pin moved to `v1.4.3` (`fae121ef`).
