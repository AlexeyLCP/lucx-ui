...
- **Runtime** (`internal/web/runtime/local.go`): delegate AWG `AddInbound`/`DelInbound` to `awg.GetManager()`. Peer add/remove: `applyLocalAwg` → `Ensure` → `awg syncconf` (fingerprint excludes peers so existing handshakes survive).
