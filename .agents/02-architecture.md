...
- **Host import** (`internal/awg/import_*.go` + `service/awg_import.go`): discover awg-multi / toolza3 / Docker Amnezia configs, preview, commit as one inbound + N clients, `Manager.Adopt` renames a live kernel iface to `awg{id}` without down/up.
...
- **qWDTT (lucx.95):** `QwdttConfig` → CLI argv (`-listen/-wg-port/-password/-dns/-listen-raw/-config-dir`) → binary `qwdtt-linux-{arch}` (GPL-3.0, external process). Settings key `lucxTunnel_qwdtt`. State dir: passwords.json + wg-keys.dat. Share: `qwdtt://config?…`, legacy `wdtt://…`, subscription JSON. **Needs root/CAP_NET_ADMIN** (TUN + MASQUERADE). Client: SpaceNeuroX Android APK. Upstream: SpaceNeuroX/proxy-turn-vk-android `./server`. **PIN (lucx.163):** build from SHA `6c2f7a62` (tag `v1.4.2`), NOT Ex3-ui `v1.0` extra-qwdtt and NOT unpinned `master` — see Pattern 1s. Lift the pin only when a newer client ships and e2e DTLS has been run.
