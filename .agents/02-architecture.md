...
- **AnyTLS (lucx.183/184):** inbound-only (`anytls-{id}`). Stock `anytls-server -l 0.0.0.0:port -p password` (anytls/anytls-go `v0.0.13`) — **no cert/key flags**, always a self-signed pair, so the share URI is `anytls://pass@host:port/?insecure=1#remark`. One password per inbound, minted on save (empty password → process stays down). `inbound.Port` is the listen port. Binary `anytls-linux-amd64` ships in the GitHub tarball and in `third_party/sidecars/` (Cores tab). Clients: sing-box / mihomo / Shadowrocket / Stash / Loon. Not in JSON/Clash sub.
