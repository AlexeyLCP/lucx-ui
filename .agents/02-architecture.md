...
- **Sidecar outbounds** (lucx.148): naive / mieru / TrustTunnel client mode. Table `sidecar_outbounds`. Each row runs a userspace client (`naiveout-N` / `mieruout-N` / `ttout-N`) that listens SOCKS on loopback; `injectSidecarOutbounds` appends an Xray `socks` outbound with the row tag **before** SOCKS/TUN egress (same order as AWG, lucx.133). Tags merge into `awgOutboundTags` for routing/balancers. Naive client is upload-only (`naive-client-*`); mieru-client and trusttunnel-client ship in the tarball. Paste `naive+https://` / `mierus://` / Throne `tt://`. Not TUN, not `injectSocksEgress`.
