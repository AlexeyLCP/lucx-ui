...
- **qWDTT (lucx.95):** `QwdttConfig` → CLI argv (`-listen/-wg-port/-password/-dns/-listen-raw/-config-dir`) → binary `qwdtt-linux-{arch}` (GPL-3.0, external process). Settings key `lucxTunnel_qwdtt`. State dir: passwords.json + wg-keys.dat. Share: single `qwdtt://config?…` (SpaceNeuroX importer; do not glue `wdtt://` onto the same clipboard — lucx.167). Legacy `wdtt://` is a separate Tunnels-card copy. Subscription JSON unchanged. **Needs root/CAP_NET_ADMIN** (TUN + MASQUERADE). Client: SpaceNeuroX Android APK. Upstream: SpaceNeuroX/proxy-turn-vk-android `./server`. **PIN (lucx.163):** build from SHA `6c2f7a62` (tag `v1.4.2`), NOT Ex3-ui `v1.0` extra-qwdtt and NOT unpinned `master` — see Pattern 1s. Lift the pin only when a newer client ships and e2e DTLS has been run.
...
- **AnyTLS (lucx.183/184/189):** inbound-only (`anytls-{id}`). LucX overlay on anytls/anytls-go `v0.0.13`: `anytls-server -l 0.0.0.0:port -p password -cert path -key path` (`third_party/patches/anytls-server-main.go`). Cert = panel ACME (`webCertFile`/`webKeyFile`) or explicit `certFile`/`keyFile`; no cert / SNI not in SAN → refuse on save, process stays down. Share URI is `anytls://pass@host:port/?sni=domain#remark` (no `insecure=1`). One password per inbound, minted on save. `inbound.Port` is the listen port. Binary `anytls-linux-amd64` ships in the GitHub tarball and in `third_party/sidecars/` (Cores tab). Clients: sing-box / mihomo / Shadowrocket / Stash / Loon. Not in JSON/Clash sub.
