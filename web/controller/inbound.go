...
func (a *InboundController) addInbound(c *gin.Context) {
	...
	// LUCX-HOOK: AWG inbounds use AWGManager.Create (interface + config + TUN + first client)
	if inbound.Protocol == "awg" {
		awgMgr := awg.NewAWGManager(&a.inboundService, &a.xrayService)
		inbound, err = awgMgr.Create(inbound)
		if err != nil {
