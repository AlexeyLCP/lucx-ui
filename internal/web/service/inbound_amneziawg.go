...
func (s *InboundService) DesiredAmneziaWGInstances() ([]amneziawg.Instance, error) {
	...
	if extra, err := s.desiredKernelAwgAsEmbedded(); err != nil {
		logger.Warning("amneziawg: kernel fallback list failed:", err)
	} else {
...
// desiredKernelAwgAsEmbedded runs LucX kernel inbounds on amneziawg-go when
// the host has no amneziawg module. No-op when the kernel path is available.
func (s *InboundService) desiredKernelAwgAsEmbedded() ([]amneziawg.Instance, error) {
	...
	if err := db.Model(model.Inbound{}).
		Where("protocol = ? AND enable = ? AND node_id IS NULL", model.AWG, true).
		Find(&inbounds).Error; err != nil {
		return nil, err
	}
	out := make([]amneziawg.Instance, 0, len(inbounds))
	for _, ib := range inbounds {
		src, ok := awg.InstanceFromInbound(ib)
		if !ok {
			continue
		}
...
func kernelAwgToEmbedded(src awg.Instance) (amneziawg.Instance, bool) {
	...
	pub := ""
	if p, err := wgutil.PublicKeyFromPrivate(src.PrivateKey); err == nil {
		pub = p
	}
	peers := make([]amneziawg.Peer, 0, len(src.Peers))
	for _, p := range src.Peers {
		...
		peers = append(peers, amneziawg.Peer{
			Email:          p.Email,
	...
	addrs := []string{}
	if src.Address != "" {
		addrs = append(addrs, src.Address)
	}
	return amneziawg.Instance{
		Id:            src.Id,
		Tag:           src.Tag,
		InterfaceName: src.Ifname,
		ListenPort:    src.Port,
		...
		Obfuscation: amneziawg.Obfuscation31{
			Jc: src.Jc, Jmin: src.Jmin, Jmax: src.Jmax,
			S1: src.S1, S2: src.S2, S3: src.S3, S4: src.S4,
			H1: src.H1, H2: src.H2, H3: src.H3, H4: src.H4,
			I1: src.I1, I2: src.I2, I3: src.I3, I4: src.I4, I5: src.I5,
			HeaderProtectionKey:    src.HeaderProtectionKey,
			ContentPaddingAddition: string(src.ContentPaddingAddition),
			RekeyAfterTime:         string(src.RekeyAfterTime),
			RekeyTimeout:           string(src.RekeyTimeout),
			RejectAfterTime:        string(src.RejectAfterTime),
			KeepaliveTimeout:       string(src.KeepaliveTimeout),
			MaxHandshakeAttempts:   string(src.MaxHandshakeAttempts),
			RandomTrailers:         src.RandomTrailers,
			DisableCookies:         src.DisableCookies,
		},
		Peers: peers,
	}, true
}
