...
func isAutoTagAnyPrefix(tag string, port, nodeID int, bits transportBits) bool {
	id := nodeID
	return isAutoGeneratedTag(tag, port, nil, bits) || isAutoGeneratedTag(tag, port, &id, bits)
}

func autoTagRenameCandidate(central []model.Inbound, snapIb *model.Inbound, origin string, snapTags map[string]struct{}, nodeID int) *model.Inbound {
	if snapIb == nil {
		return nil
	}
	bits := inboundTransports(snapIb.Protocol, snapIb.StreamSettings, snapIb.Settings)
	if !isAutoTagAnyPrefix(snapIb.Tag, snapIb.Port, nodeID, bits) {
		return nil
	}
	var hit *model.Inbound
	for i := range central {
		c := &central[i]
		if _, kept := snapTags[c.Tag]; kept {
			continue
		}
		if c.Protocol != snapIb.Protocol {
			continue
		}
		if strings.TrimSpace(c.Listen) != strings.TrimSpace(snapIb.Listen) {
			continue
		}
		cOrigin := c.OriginNodeGuid
		if cOrigin == "" {
			cOrigin = origin
		}
		if cOrigin != origin {
		...
		cBits := inboundTransports(c.Protocol, c.StreamSettings, c.Settings)
		if !isAutoTagAnyPrefix(c.Tag, c.Port, nodeID, cBits) {
			continue
		}
		if hit != nil {
			return nil
...
func (s *InboundService) setRemoteTrafficLocked(nodeID int, snap *runtime.TrafficSnapshot, dirty, justPushed bool) (bool, error) {
	...
	for _, snapIb := range snap.Inbounds {
		...
		if !ok {
			...
			case 0:
				if renamed := autoTagRenameCandidate(central, snapIb, origin, snapTags, nodeID); renamed != nil {
					c, ok = renamed, true
					tagToCentral[snapIb.Tag] = c
					snapTags[c.Tag] = struct{}{}
				}
		...
		if !dirty {
			...
			if c.Tag != snapIb.Tag && c.Port != snapIb.Port {
				oldBits := inboundTransports(c.Protocol, c.StreamSettings, c.Settings)
				newBits := inboundTransports(snapIb.Protocol, snapIb.StreamSettings, snapIb.Settings)
				if isAutoTagAnyPrefix(c.Tag, c.Port, nodeID, oldBits) &&
					isAutoTagAnyPrefix(snapIb.Tag, snapIb.Port, nodeID, newBits) {
					want := composeInboundTag(snapIb.Port, c.NodeID, newBits)
					var owner model.Inbound
					err := tx.Where("tag = ? AND id <> ?", want, c.Id).First(&owner).Error
					if errors.Is(err, gorm.ErrRecordNotFound) {
						updates["tag"] = want
					}
				}
