...
Данное ПО предоставляется «как есть», без каких-либо гарантий, явных или подразумеваемых. Оно создано исключительно в образовательных, исследовательских целях и для оптимизации сетей с целью изучения поведения зашифрованных протоколов в условиях ограниченной пропускной способности.

Автор:
1. Не предоставляет коммерческих прокси, VPN или услуг передачи данных.
2. Не обслуживает, не контролирует и не отслеживает серверы, развёрнутые третьими лицами с использованием данного ПО.
3. Не несёт ответственности за неправомерное использование, незаконное распространение или нарушение местного законодательства конечными пользователями.

Вся ответственность за развёртывание, использование и соблюдение законодательства лежит исключительно на лице, запускающем установочный скрипт.
...
URL и реквизиты доступа выводятся установщиком в конце. Логин, пароль, порт и basepath генерируются случайно при каждой установке.

### Требования

Debian 12/13, Ubuntu 22.04/24.04. Go 1.24+, Node.js 18+ для сборки из исходников.
...
- 6 протоколов с пресетами обхода ТСПУ (Россия, май 2026) — применение в один клик во вкладке Stream
- Автоматические дочерние Xray-инбаунды — TUN для AWG, SOCKS5 для Telemt
- Нативный учёт трафика через gRPC API Xray, без самописных парсеров
- 152 LUCX-HOOK маркера — чистые слияния с апстримом, ~31 Go + 9 Vue/JS изолированных файлов
- 68 тестов — модульные, интеграционные, хаос/стресс
